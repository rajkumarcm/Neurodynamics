...
Simulates two layers of QIF neurons. Layer 0 is stimulated
...
plt.subplot(211)
plt.plot(range(T), v1)
plt.title('QIF - Population 1 membrane potentials')
plt.ylabel('Voltage (mV)')
plt.ylim([-90, 40])

plt.subplot(212)
plt.plot(range(T), v2)
plt.title('QIF - Population 2 membrane potentials')
plt.ylabel('Voltage (mV)')
plt.ylim([-90, 40])
plt.xlabel('Time (ms)')
...
plt.ylim(0, N1+1)
plt.title('QIF - Population 1 firings')
...
plt.ylim(0, N2+1)
plt.xlabel('Time (ms)')
plt.title('QIF - Population 2 firings')
